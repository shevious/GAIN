...
class H_layer(keras.layers.Layer):
    """Uses (z_mean, z_log_var) to sample z, the vector encoding a digit."""

    def call(self, inputs):
        x = inputs
        shape = tf.shape(x)
        Z = keras.backend.random_uniform(shape=shape, minval=0.0, maxval=0.01)
        H_rand = tf.keras.backend.random_normal(shape=shape)
        H_temp = tf.where(H_rand < 0.9, 1., 0.)
        return Z, H_temp

class GAIN(keras.Model):
  def __init__(self, shape, alpha=100., load=False, hint_rate=0.9, **kwargs):
...
#      rmse = tf.sqrt(tf.reduce_sum(tf.where(M == 0, G_sample - Y, 0.)**2)/tf.reduce_sum(1-M))
  ...
  def call(self, inputs):
      ...
      isnan = tf.math.is_nan(x)
      m = keras.backend.cast(isnan, dtype=tf.float32)
...
def gain (data_x, gain_parameters):
  ...
  #n = X_mb.shape[0]
  #isnan = (M_mb == 0)
  #X_mb = tf.where(isnan, np.nan, X_mb)
  
  ## Iterations
  gain= GAIN(dim, alpha, load=False)
  gain.compile()
  gain.fit(ds, batch_size=128, steps_per_epoch = 10000, epochs=1)
  #import sys
  #sys.exit(0)
  #gain.predict(X_mb)
